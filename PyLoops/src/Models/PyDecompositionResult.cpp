...
void PyLoops::models::PyDecompositionResult::registerPy(pybind11::module& mod)
{
    ...
    .def("recomputeObjectiveValue",&DC::recomputeObjectiveValue)
    .def("pruneToTopWeighted", &DC::pruneToTopWeighted)
    ...
    .def("applyNNLSAndPruneBatched", [](DC& dc, double pruneValue, std::size_t batchSize) {
        pybind11::scoped_ostream_redirect streamRedir(
            std::cout,                               // std::ostream&
            pybind11::module::import("sys").attr("stdout") // Python output
        );
        dc.applyNNLSAndPruneBatched(pruneValue, batchSize);
    })
    .def("applyNNLSIncrementally", [](DC& dc, std::size_t batchSize, std::size_t retainSize, bool retainFraction) {
        pybind11::scoped_ostream_redirect streamRedir(
            std::cout,                               // std::ostream&
            pybind11::module::import("sys").attr("stdout") // Python output
        );
        return dc.applyNnlsIncrementally(batchSize, retainSize, retainFraction);
    })
    .def("greedyRefine", [](DC& dc) {
        pybind11::scoped_ostream_redirect streamRedir(
            std::cout,                               // std::ostream&
            pybind11::module::import("sys").attr("stdout") // Python output
        );
        LoopsLib::Algs::NNLS nnls;
        nnls.greedyRefine(dc.m_basis, dc.field(), dc.m_decompositionCoeffs,dc.m_objectValueNNLS);
    })
    ...
    .def("computeMinFrechetRepresentatives", [](DC& dc, LoopsLib::NT maximumValue) -> std::vector<LoopsLib::NT>
    {
        LoopsLib::Algs::Processing::TrajectorySetFrechet setFrechet;
        std::vector<LoopsLib::NT> out;
        setFrechet.minFrechetValues(dc.m_relatedInstance->m_graph, dc.m_relatedInstance->m_representatives, dc.m_basis, maximumValue, out);
        return out;
    })
    .def("computeReverseMinFrechet",[](DC& dc, LoopsLib::NT maximumValue)
    {
        LoopsLib::Algs::Processing::TrajectorySetFrechet setFrechet;
        std::vector<LoopsLib::NT> out;
        std::vector<std::size_t> indices;
        setFrechet.minFrechetValues(dc.m_relatedInstance->m_graph, dc.m_basis,dc.m_relatedInstance->m_field->m_paths, maximumValue, out,indices);
        return std::make_pair(out, indices);
    })
    .def("computeReverseMinFrechetRepresentatives", [](DC& dc, LoopsLib::NT maximumValue)
    {
        LoopsLib::Algs::Processing::TrajectorySetFrechet setFrechet;
        std::vector<LoopsLib::NT> out;
        setFrechet.minFrechetValues(dc.m_relatedInstance->m_graph, dc.m_basis, dc.m_relatedInstance->m_representatives, maximumValue, out);
        return out;
    });
}
